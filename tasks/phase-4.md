...
- [x] 첨부 파일: csv, xlsx 지원
- [x] 첨부 파일 확장: pdf/docx/pptx/md/txt + 이미지(vision 멀티모달)
...
- [x] **이미지 (.png, .jpg, .jpeg, .webp, .gif)**
  - 처리 방식: 프로젝트 첨부로 저장(텍스트 추출은 하지 않음) + 모델 호출 시점에 로컬 파일을 읽어 base64로 변환해 멀티모달(vision) 입력으로 전달
  - 구현:
    - Rust: 이미지 확장자는 `attach_file`에서 Unsupported로 막지 않음(텍스트는 빈 문자열)
    - Rust: `read_file_bytes`로 로컬 파일 바이트를 반환
    - Front: base64로 변환 후 LangChain 표준 content blocks(`type: image`, `source_type: base64`)로 HumanMessage에 포함
...
- [x] **Tauri Commands**
  ...
  - `read_file_bytes`: 로컬 파일 바이트 읽기(이미지 멀티모달 입력용)
  - `extract_file_text`: 파일 타입별 텍스트 추출 로직

#### 4.2.4 구현 우선순위 및 세부 단계

1.  **단계 1: 인프라 구축**
    - [x] SQLite `attachments` 테이블 추가 (`src-tauri/src/db/schema.rs`)
    - [x] `Attachment`/`AttachmentDto` 모델 정의 (`src-tauri/src/models.rs`)
    - [x] Tauri Commands 구현 (`attach_file`, `list_attachments`, `delete_attachment`, `read_file_bytes`)

2.  **단계 2: 텍스트 추출 엔진 (Rust)**
    - [x] **Markdown (.md)**: `std::fs::read_to_string`
    - [x] **PDF (.pdf)**: `pdf-extract`
    - [x] **Word (.docx)**: `docx-rs`
    - [x] **PowerPoint (.pptx)**: Zip 파일 내 XML 파싱 (슬라이드 텍스트 추출)

3.  **단계 3: 프롬프트 연동**
    - [x] `src/ai/prompt.ts`의 `buildLangChainMessages`: 첨부 파일 텍스트 주입 로직 추가
    - [x] 길이 제한(전체 8,000자) 적용 및 초과 시 절단

4.  **단계 4: UI 구현 (React)**
    - [x] `ChatPanel.tsx`에 첨부 파일 업로드 및 목록 표시 UI 추가
    - [x] 파일 선택기 연동 (`@tauri-apps/plugin-dialog`)
    - [x] 업로드 중 로딩 상태 및 에러 표시

5.  **단계 5: 이미지 지원**
    - [x] 이미지 첨부 허용 및 멀티모달(vision) 입력 연동
