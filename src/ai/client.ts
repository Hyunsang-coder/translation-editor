...
import { getAiConfig, type ModelRunConfig, type ModelUseFor } from '@/ai/config';
...
export function createChatModel(
  ...
  /** 문서 한 벌을 통째로 뱉는 용도 — 출력 상한을 크게 잡아야 한다. */
  const isLongOutput = useFor === 'translation' || useFor === 'review' || useFor === 'polish';
  ...
  if (cfg.provider === 'anthropic') {
    ...
    // Claude는 max_tokens 기본값이 낮으므로 명시적 설정 (검수·폴리싱은 번역과 동일 취급)
    const maxTokensOption = options?.maxTokens
      ? { maxTokens: options.maxTokens }
      : (isLongOutput ? { maxTokens: DEFAULT_TRANSLATION_MAX_TOKENS } : { maxTokens: DEFAULT_CHAT_MAX_TOKENS });
  ...
  if (cfg.provider === 'openai' || cfg.provider === 'mock') {
    ...
    return new ChatOpenAI({
      ...
      // summary는 채팅 경로에서 파생된 호출이라 같은 API를 쓴다(reasoning 파라미터 형태가 동일).
      ...(useFor === 'chat' || useFor === 'summary' ? { useResponsesApi: true } : {}),
      ...(tauriFetch
