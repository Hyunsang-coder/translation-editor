...
function buildTranslationSetup(params: {
  ...
  const direction = resolveDirection(
    {
      source: params.project.metadata.sourceLanguage,
      target: params.project.metadata.targetLanguage,
    },
    sourceMarkdown,
  );
  const srcLang = direction.source.language ?? 'Source';
  const tgtLang = direction.target.language ?? 'Target';
