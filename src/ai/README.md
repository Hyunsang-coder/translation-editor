...
- **Secondary(최소 지침)**: 각 도구의 `description` + `prompt.ts`의 system message
  ...
  - 안전장치 예:
    ...
    - 응답에서 “저장/추가 완료” 금지
    - 필요 시 특정 라벨을 하드코딩하지 않고 **제안 카드의 승인 버튼** 안내
...
정책:
- **질문/검수에 꼭 필요할 때만** 호출합니다. 선택 요청에서는
  `get_aligned_selection_context`를 먼저 쓰고, 전체 문서는 선택 문맥으로 답할 수 없을 때만 조회합니다.
- 선택이 없는 일반 대조/검수 요청은 문서 조회 도구를 먼저 호출합니다.
...
전체 번역과 문서 폴리싱 결과는 프리뷰 전에 `documentIntegrity.ts`의 하드 게이트를 통과해야 합니다.

- TipTap 블록 topology와 heading/list/code 구조
- 표의 행·열·셀 타입·병합·`colwidth`
- 이미지 개수·위치·순서·원본 attrs
- 링크 href와 bold/italic 등 인라인 mark
- 코드·URL·숫자·날짜·버전·placeholder
- `translationUnitId`

한 항목이라도 달라지면 결과 전체를 차단합니다. 실행:
