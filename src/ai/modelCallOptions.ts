...
/** Opus 4.7 이상(4.7/4.8/4.9 및 2자리 이상 버전) 및 Opus 5+ (5-5 포함) */
...
export function modelSupportsEffort(model: string): boolean {
  return isOpus47Plus(model) || isSonnet5(model) || model.startsWith('gpt-5') || model.startsWith('gpt-6');
}
 ...
 *
 * 규칙:
 * - temperature: Anthropic Opus 4.7+/Sonnet 5, OpenAI gpt-5·gpt-6 계열은 non-default를
 ...
 *   Rust 경로(commands/ai.rs)의 starts_with("gpt-5")/("gpt-6") 판정과 동일 기준. (A3)
 */
export function resolveModelCallOptions(cfg: AiConfig): ModelCallOptions {
  ...
  const isGptReasoning = cfg.model.startsWith('gpt-5') || cfg.model.startsWith('gpt-6');
  if (!isGptReasoning && cfg.temperature !== undefined) {
    opts.temperature = cfg.temperature;
  }
  // effort는 reasoning_effort를 지원하는 gpt-5/gpt-6 계열에만 포함한다.
  // 호출 경로(client.ts / backendCompletion.ts)는 이 결과를 신뢰해 그대로 전달한다. (A3)
  if (isGptReasoning && cfg.reasoningEffort) {
