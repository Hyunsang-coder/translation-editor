...
describe('estimateCost', () => {
  it('입력/출력/캐시를 각각의 단가로 환산한다', () => {
    ...
    expect(cost.inputUsd).toBeCloseTo(4, 6);
    expect(cost.outputUsd).toBeCloseTo(20, 6);
    expect(cost.cacheReadUsd).toBeCloseTo(0.2, 6); // $4의 0.05배(출시일 보도 확인값)
    expect(cost.cacheWriteUsd).toBeCloseTo(5, 6); // 정가의 1.25배
    expect(cost.totalUsd).toBeCloseTo(29.2, 6);
  });
  ...
  it('캐시분을 정가 입력에서 빼고 센다(이중 계상 금지)', () => {
    ...
    expect(cost.totalUsd).toBeCloseTo(0.2, 6);
  });
  ...
  it('캐시 절감액은 정가로 냈을 때와의 차액이다', () => {
    ...
    // 정가 $4 대신 $0.2를 냈으므로 $3.8 절감.
    expect(cost.cacheSavingsUsd).toBeCloseTo(3.8, 6);
  });

  it('OpenAI GPT-6 출시 단가를 쓴다', () => {
    const cost = estimateCost('gpt-6-luna', {
    ...
    expect(cost.inputUsd).toBeCloseTo(0.1, 6); // 정가 1M × $0.10
    expect(cost.outputUsd).toBeCloseTo(0.5, 6);
    expect(cost.cacheReadUsd).toBeCloseTo(0.01, 6); // 정가의 0.1배
    // OpenAI는 캐시 write 과금이 없고 provider도 cache_creation을 보고하지 않는다.
    expect(cost.cacheWriteUsd).toBe(0);
    expect(cost.totalUsd).toBeCloseTo(0.61, 6);
  });
