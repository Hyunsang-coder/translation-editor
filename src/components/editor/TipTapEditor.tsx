...
function TipTapEditor({
  ...
  const richHtmlPasteRef = useRef(false);
  ...
  const editor = useEditor({
    ...
    editorProps: {
      ...
      transformPastedHTML: (html) => {
        // WebKit/macOS가 Confluence HTML을 Word 계열 HTML로 재표현하기도 하므로
        // Atlassian 전용 표식에 의존하지 않고 모든 서식 HTML의 최종 Slice를 정리한다.
        richHtmlPasteRef.current = html.trim().length > 0;
      ...
      transformPasted: (slice) => {
        const shouldNormalizeSlice = richHtmlPasteRef.current;
        richHtmlPasteRef.current = false;
        return shouldNormalizeSlice ? normalizeConfluencePastedSlice(slice) : slice;
      },
