...
문제가 없으면: { "issues": [] }`;
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col bg-editor-bg">
      {/* 진행 상태 바 */}
      {isReviewing && (
        <div className="px-4 py-2 border-b border-editor-border bg-editor-surface/50">
          <div className="flex items-center justify-between text-xs text-editor-muted mb-1">
            <span>{t('review.progress', '검수 진행 중...')}</span>
            ...
            <button
              ...
              className={`
                ...
                ${highlightEnabled
                  ...
                  : 'border border-editor-border hover:bg-editor-bg disabled:opacity-50'
                }
              `}
              title={highlightEnabled
                ? t('review.hideHighlight', '하이라이트 숨기기')
                : t('review.showHighlight', '에디터에서 표시')
              }
            >
              ...
              {highlightEnabled
                ? t('review.highlighting', '표시 중')
                : t('review.highlight', '표시')
              }
            </button>
          )}
        </div>
