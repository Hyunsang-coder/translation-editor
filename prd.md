...
**Source 참조 패널**:
- 원문은 왼쪽에 읽기 전용으로 배치
- AI에게는 항상 최우선 맥락(Context)으로 자동 제공
...
- 번역문 수정은 99% 사용자가 직접 수행 (추후 전체 덮어쓰기 기능 예정)
- AI 응답은 간결하게 - 번역 요청 시 번역문만, 질문 시 답변만

**Add to Chat**:
- 선택한 텍스트를 채팅 입력창에 추가하는 보조 UX
- 모델 호출 없이 컨텍스트 준비 용도

**채팅 세션 분리 권장**:
- 번역용 탭: 번역 요청 및 수정 관련 대화
- 질문용 탭: 내용/문맥/용어에 관한 질문
...
**Phase 2**:
- 전체 덮어쓰기 기능 (AI 번역문으로 Target 전체 교체)
