...
#### Issue #5: SecretsInitResult 필드명 불일치
- **위치**: `secrets.rs` 라인 22-25, `secrets.ts` 라인 22-25
- **문제**: Rust는 `success`/`cachedCount`(camelCase), TS는 `initialized`/`cached_count` 기대 → 런타임에 undefined
- **해결**: TS 인터페이스를 Rust JSON 출력에 맞게 수정
- **상태**: [x] 완료 (2026-01-07)

#### Issue #6: Vault 복호화 실패 시 조용히 덮어쓰기
- **위치**: `manager.rs` 라인 170-173
- **문제**: vault 복호화 실패 시 로그만 출력하고 Ready 진입 → 새 시크릿 저장 시 기존 vault 덮어쓰기 (토큰 손실)
- **해결**: 복호화 실패 시 `VaultDecryptFailed` 에러 반환, `InitState::Failed` 상태로 전환하여 토큰 보호
- **상태**: [x] 완료 (2026-01-07)

#### Issue #7: 초기화 대기 무한 루프
- **위치**: `manager.rs` 라인 111-120
- **문제**: 첫 번째 호출자가 panic/hang 시 다른 호출자가 영원히 대기
- **해결**: 최대 30초 timeout 추가
- **상태**: [x] 완료 (2026-01-07)

### Low Priority

#### Issue #8: SSE 타임아웃 시 좀비 태스크
- **위치**: `client.rs` 라인 254-262
- **문제**: SSE 엔드포인트 대기 타임아웃 시 에러 반환하지만 백그라운드 SSE 태스크는 계속 실행 → 연결 누수
- **해결**: 타임아웃 시 shutdown_tx를 통해 SSE 태스크에 종료 signal 전송
- **상태**: [x] 완료 (2026-01-07)

#### Issue #9: save_master_key_to_keychain 실패 시 상태 미전환
- **위치**: `manager.rs` 라인 156
- **문제**: 새 마스터키 생성 후 키체인 저장 실패 시 `?`로 에러 반환하지만 InitState가 Initializing에 머물러 이후 호출이 무한 대기
- **해결**: 저장 실패 시 명시적으로 `InitState::Failed`로 전환
- **상태**: [x] 완료 (2026-01-07)

#### Issue #10: OAuth 콜백 서버 타임아웃 미종료
- **위치**: `oauth.rs` 라인 330, 392
- **문제**: OAuth 흐름 타임아웃 또는 open::that 실패 시 콜백 서버가 포트 23456을 계속 점유 → 다음 인증 시도 시 바인드 실패
- **해결**: 콜백 서버에 6분 자체 타임아웃 추가, accept에 10초 타임아웃 적용하여 주기적 체크
- **상태**: [x] 완료 (2026-01-07)

#### Issue #11: 초기화 대기 타임아웃 키체인 프롬프트 고려
- **위치**: `manager.rs` 라인 114
- **문제**: 30초 타임아웃이 키체인 프롬프트 응답 대기 중 발생하면 false positive 에러
- **해결**: 타임아웃을 60초로 증가, 에러 메시지에 키체인 프롬프트 관련 안내 추가
- **상태**: [x] 완료 (2026-01-07)

#### Issue #12: 초기화 타임아웃 후 재시도 불가
- **위치**: `manager.rs` 라인 127-132
- **문제**: 타임아웃 발생해도 첫 번째 초기화의 상태가 Initializing에 남아 재시도 시에도 타임아웃됨
- **해결**: 60초 타임아웃 시 상태를 `NotInitialized`로 리셋하여 재시도 가능하게 함
- **상태**: [x] 완료 (2026-01-07)
