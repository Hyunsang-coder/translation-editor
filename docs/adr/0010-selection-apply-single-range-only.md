...
## 개정 4 (2026-09-15): 못 읽은 블록만 원문 유지한다

**결정**: `runSegments` 파싱을 관대하게 바꾼다 — 1-based 번호·INPUT 마커 에코는 정규화하고, 못 읽은 블록(빈 블록·잘림·중간 누락)은 그 블록의 원문(`currentTargetText`)으로 유지한다. 마커가 하나도 없을 때만 응답 앞부분을 담은 에러를 던진다.

**버린 것(개정 1·3의 "하나라도 비면 전부 버린다")**: 엄격 모드는 일시적인 마커 glitch 하나를 전체 실패로 증폭시켜, 멀쩡한 블록까지 날아가 "자꾸 실패"가 됐다(불릿 여러 개 선택이 대표 사례). 원문 유지는 날조가 아니라 status quo이고, 모달이 원문과 같은 제안을 "변경 없음"으로 보여주는 기존 경로와 같은 값이라 적용 단계도 그대로 탄다. Preview-First라 사용자가 보기 전에 문서에 들어가지 않는다.
