...
#### 리치 텍스트 입력 (Rich Text Composer)
- **TipTap 기반 에디터**: 채팅 입력창은 TipTap 리치 텍스트 에디터를 사용한다.
- **지원 서식**:
  - 굵게 (Bold): `**텍스트**` 또는 Cmd+B
  - 기울임 (Italic): `*텍스트*` 또는 Cmd+I
  - 밑줄 (Underline): Cmd+U
  - 하이라이트: 배경색 강조
  - 링크: URL 자동 감지 또는 수동 삽입
  - 인라인 코드: `` `코드` ``
  - 리스트: `-` 또는 `1.`로 시작
- **실시간 렌더링**: 입력 시 서식이 즉시 적용되어 표시됨
- **Markdown 중간 형식**: 내부적으로 Markdown을 저장/전송 (SQLite 호환성 100% 유지)
- **IME 호환**: 한글 입력 중 Enter 키 무시 (조합 완료 후에만 전송)
- **구현 파일**: `src/components/chat/ChatComposerEditor.tsx`

#### 에디터 번역 버튼 주변
- 번역 모델 선택 드롭다운은 Editor의 "번역" 버튼 **왼쪽**에 둔다.
- 기본 모델은 `gpt-5.2`, 프리셋 외 커스텀 모델 입력은 지원하지 않는다.
...
- **웹검색 도구**: OpenAI 내장 `web_search_preview` 도구 사용 (Responses API)
- **Provider 제한**: 웹검색은 **OpenAI Provider에서만** 사용 가능
  - Anthropic Provider 선택 시 웹검색 토글이 비활성화됨
...
## 4.9 이미지 업로드 (Chat Composer)

### Why
- 번역 작업 시 스크린샷, 참조 이미지 등을 AI에게 직접 보여주면 더 정확한 맥락 파악이 가능합니다.
- 이미지 드래그 앤 드롭, 붙여넣기 등 직관적인 UX를 제공합니다.
...
- **이미지 미리보기**: 업로드된 이미지는 입력창 위에 썸네일로 표시
- **삭제**: 썸네일의 X 버튼으로 개별 삭제 가능
- **전송**: 이미지와 함께 메시지 전송 시 멀티모달 메시지로 처리
...
### Why
- AI 응답을 다른 곳에 활용하기 위해 쉽게 복사할 수 있어야 합니다.
- 코드 블록, 전체 메시지 등 다양한 복사 옵션을 제공합니다.

### What
- **메시지 복사**: 각 AI 메시지에 복사 버튼 제공
  - 마우스 호버 시 복사 버튼 표시
  - 클릭 시 메시지 전체 내용을 클립보드에 복사
  - 복사 완료 시 체크 아이콘으로 피드백
- **코드 블록 복사**: 코드 블록 내 별도 복사 버튼
  - 코드 블록 우상단에 복사 버튼 표시
  - 코드 내용만 복사
- **에디터 복사**: 에디터 캔버스에서도 복사 기능 지원
  - 선택 영역 복사 (Cmd+C / Ctrl+C)
  - 전체 복사 버튼
