...
156. **타겟 언어는 날값으로 읽지도, 문자열로 비교하지도 말 것**: `project.metadata.targetLanguage`의 기본값은 센티널 `'auto'`다([ADR-0020](../docs/adr/0020-auto-target-language.md)). 프롬프트·MCP로 흘리기 전에 반드시 `resolveTargetLanguage(stored, sourceText)`를 거칠 것 — 안 거치면 `"Source에서 auto로 번역하세요"`가 모델에 간다. 비교는 `normalizeLang()`을 거칠 것: Select 값은 한글 라벨(`'한국어'`), `detectSourceLanguage`는 영문명(`'Korean'`)이라 `===`로는 **동일언어 가드가 영원히 안 걸린다**. 브리지(`oddeyesAppBridge`)는 저장값이 아니라 해석값을 내보내야 한다 — 센티널을 주면 외부 에이전트의 방향 교차검증이 통째로 꺼진다.
