...
115. **`<pre>` 없는 블록 코드 붙여넣기**: Confluence Cloud는 코드 블록을 `<pre>` 없이 `<code class="language-" style="… white-space: pre …">`(줄마다 `span[data-ds--code--row]`, 행 번호는 CSS 배경)로 복사한다. 승격하지 않으면 개행을 품은 채 문단 속 **인라인 code mark**가 되어 화면에서 한 줄로 접힌다. `htmlNormalizer.promoteBlockCode()`가 `white-space: pre*` 또는 code row를 신호로 `<pre><code>`로 감싸고 빈 `language-` 클래스를 버린다. **DOMPurify가 `style`을 제거하므로 style 기반 정규화는 반드시 sanitize 전에 돌려야 한다.** 실물 클립보드 픽스처는 `src/utils/__fixtures__/confluenceClipboard.ts`.
