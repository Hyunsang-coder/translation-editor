...
150a. **세그먼트 마커 파싱을 엄격하게 짜지 말 것**: 모델은 1-based 번호·INPUT 에코·빈 블록·잘림·마커 생략을 간헐적으로 내놓는다. 하나라도 못 읽으면 전부 버리면 멀쩡한 블록까지 날아가 "자꾸 실패"가 된다(`runSegments` 구 동작). 계약은 정규화 후 블록별 원문 유지, 마커 전무 때만 스니펫 포함 throw다. 빈 블록 금지는 프롬프트(`never leave one empty`)가 아니라 코드가 보장해야 한다 — 지시는 위반된다. 안전망은 Preview-First + 모달의 "변경 없음" 표시다.
...
26. **jsdom AbortSignal은 undici fetch가 거부한다**: vitest jsdom에서 LangChain SDK 실호출(`*.live.test.ts`)을 하면 SDK 내부의 jsdom 클래스 signal 때문에 `RequestInit: Expected signal ... to be an instance of AbortSignal`으로 전멸한다. `src/test/liveFetchPatch.ts`의 `patchFetchForLiveTests()`를 live 파일 최상단에서 호출할 것(비-네이티브 signal만 제거, vitest 타임아웃이 행잉 가드). `new Request(...).signal.constructor`로 네이티브 클래스를 빼오는 꼼수는 통하지 않는다 — undici가 만든 signal도 전역 클래스를 따른다.
...
115. **`<pre>` 없는 블록 코드 붙여넣기**: Confluence Cloud는 코드 블록을 `<pre>` 없이 `<code class="language-" style="… white-space: pre …">`(줄마다 `span[data-ds--code--row]`, 행 번호는 CSS 배경)로 복사한다. 승격하지 않으면 개행을 품은 채 문단 속 **인라인 code mark**가 되어 화면에서 한 줄로 접힌다. 반대로 한 줄 인라인 코드에도 `white-space: pre-wrap`이 붙으므로 스타일만 보고 승격하면 안 된다. `htmlNormalizer.promoteBlockCode()`는 code row가 있거나 `white-space: pre*`와 실제 개행이 함께 있을 때만 `<pre><code>`로 감싸고 빈 `language-` 클래스를 버린다. `<p><span><code>…` 구조에서는 pre를 문단 밖으로 꺼내지 않으면 HTML 재파싱이 앞뒤 빈 문단을 만든다. **DOMPurify가 `style`을 제거하므로 style 기반 정규화는 반드시 sanitize 전에 돌려야 한다.** 실물 클립보드 픽스처는 `src/utils/__fixtures__/confluenceClipboard.ts`.

116. **plain text 붙여넣기의 기본 동작은 "줄마다 문단"**: ProseMirror 클립보드 텍스트 파싱은 빈 줄 그룹핑/hardBreak가 아니라 줄마다 별도 문단을 만들고 빈 줄을 버린다(bare StarterKit에서도 동일). `plainTextPaste.ts`는 이 파리티를 지키면서 코드 펜스/인라인 백틱/체크박스만 얹는다. 펜스로 구간을 먼저 자르므로 펜스 내부의 `[]byte`·`[x]`가 태스크리스트로 오변환되지 않는다(펜스 밖 한 줄 코드는 기존 오탐이 남아 있다). 코드 들여쓰기·개행 보존에는 `insertContent(html, { parseOptions: { preserveWhitespace: 'full' } })`가 필수다.

117. **Confluence sticky table header와 빈 문단**: 긴 표를 복사하면 고정 헤더 복제본(헤더 한 줄뿐인 별도 table), zero-width 문자만 든 문단, 실제 표가 연속으로 올 수 있다. `removeEmptyParagraphs()`는 `U+200B–U+200D`, `U+2060`, `U+FEFF`도 공백으로 보고 제거해야 하며, 그래야 `removeDuplicateTableHeaders()`가 복제본과 실제 표를 인접 요소로 비교해 중복 헤더를 지울 수 있다. 최상위 HTML 들여쓰기 공백도 `preserveWhitespace` 파싱에서 빈 paragraph가 되므로 code/table 밖의 최상위 공백 텍스트를 제거한다.

118. **스트리밍 전 선행 await(대화 요약)는 세션 스코프 로딩에 안 보인다**: `useSessionStreamingState`는 `streamingSessionId === sessionId`일 때만 `isLoading`/`statusMessage`를 반환하는데, `streamingSessionId`는 assistant placeholder 생성 때 세팅되므로 요약 `await` 동안은 스켈레톤도 취소도 없이 전송만 무시되는 먹통이 된다. 선행 단계가 있으면 요청 시작 시점에 `streamingSessionId`를 먼저 세팅하고(`chatStore.ai.ts`), non-streaming 내부 호출(`summarizeConversation`)에는 전체 타임아웃(`SUMMARY_TIMEOUT_MS`)+입력 상한(`MAX_SUMMARY_INPUT_TOKENS`, 오래된 쪽 청크·throughId는 자른 끝까지만)을 둘 것. `withRetry`의 sleep은 signal을 받아야 취소가 수 초 지연되지 않는다.
