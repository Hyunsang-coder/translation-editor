...
**현재 도구** (`oddeyes-desktop-mcp` v1.0.1, 23개):
...
- 프로젝트 메모리: `list_project_memory`, `add`/`replace`/`delete_project_memory_item`, `upsert_forbidden_term` / `delete_forbidden_term`
...
- **Segment marker parsing is lenient; original text is the fallback.** The model intermittently returns 1-based numbering, INPUT-marker echoes, empty blocks (a legitimate "no change" polish outcome), truncation before the last END, or bare text with no markers. `runSegments` normalizes the first two and keeps the original `currentTargetText` for any unreadable block — it throws only when no markers exist at all (with a response snippet). Single-selection `extractReplacement` accepts a bare non-empty response outright (one block, no split risk). Never reintroduce all-or-nothing parsing: one glitchy block used to discard every good block ("N번째 블록 누락"). The safety net is Preview-First plus the modal's "변경 없음" display, not parser strictness.
...
- **Mixed inline formatting is confirmed, not refused.** `selectionHasUniformFormatting` gates the retranslate entry point: uniform → straight through; mixed → a native `confirm` warning that some formatting may be lost, and consent sets `flattenFormatting` on the request. Apply then keeps only the marks common to *every* text node in the range (`getCommonSelectionMarks`) instead of the start-node marks — an italic sentence with one bold span stays italic and loses the bold. Without consent `applySelectionEdit` still returns `formatting-conflict`, which guards the case where the document's formatting changed while the modal was open.
- The preview modal owns three exits, not two: apply, **regenerate** (`onGenerate` stays reachable after a proposal exists), and manual edit (`onReplacementChange` swaps the diff view for a textarea). Typing an instruction must *not* clear `replacementText` — that would discard hand edits; regeneration is only ever explicit. Chat's proposal-only preview passes neither callback and stays read-only.
- Counterpart source lookup goes through `findAlignedCounterpartUnits` (`editor/utils/alignedCounterpartUnits.ts`) — ID match first, then the alignment view's LCS for legacy documents. Same function, argument order reversed, serves chat context and review position hints.
