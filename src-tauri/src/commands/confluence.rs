//! Confluence REST API Commands
//!
//! MCP OAuth 토큰을 재사용하여 Confluence REST API 직접 호출.
//! 단어 카운팅 등 LLM 컨텍스트에 내용을 노출하지 않아야 하는 작업에 사용.

use crate::mcp::client::MCP_CLIENT;
use once_cell::sync::Lazy;
use serde::{Deserialize, Serialize};
use tracing::{debug, info, warn};

/// 재사용 HTTP 클라이언트 (커넥션 풀링)
static HTTP_CLIENT: Lazy<reqwest::Client> = Lazy::new(reqwest::Client::new);

/// Confluence 페이지 콘텐츠 응답
#[derive(Debug, Serialize, Deserialize)]
pub struct ConfluencePageContent {
    pub page_id: String,
    pub title: String,
    /// storage 포맷 (HTML)
    pub body: String,
}

/// Confluence REST API v2 페이지 응답 구조
#[derive(Debug, Deserialize)]
struct ConfluenceApiPageResponse {
    id: String,
    title: String,
    body: Option<ConfluenceApiBody>,
}

#[derive(Debug, Deserialize)]
struct ConfluenceApiBody {
    storage: Option<ConfluenceApiStorage>,
}

#[derive(Debug, Deserialize)]
struct ConfluenceApiStorage {
    value: String,
}

/// Accessible Resources 응답 구조
#[derive(Debug, Deserialize)]
struct AccessibleResource {
    id: String,
}

/// Validate that a string contains only safe URL path characters (alphanumeric, hyphens).
/// Confluence page IDs are numeric; cloud IDs are UUIDs (alphanumeric + hyphens).
fn validate_url_segment(value: &str, name: &str) -> Result<(), String> {
    if value.is_empty() {
        return Err(format!("{} is empty", name));
    }
    if !value
        .chars()
        .all(|c| c.is_ascii_alphanumeric() || c == '-')
    {
        return Err(format!("{} contains invalid characters: {}", name, value));
    }
    Ok(())
}

/// Confluence 페이지 HTML(storage format) 가져오기
///
/// MCP OAuth 토큰을 재사용하여 Confluence REST API v2 직접 호출.
/// 결과는 Tauri command로만 반환되어 LLM 컨텍스트에 노출되지 않음.
#[tauri::command]
pub async fn confluence_get_page_html(page_id: String) -> Result<ConfluencePageContent, String> {
    // Validate page_id before any use
    validate_url_segment(&page_id, "page_id")?;

    debug!("[Confluence REST] Getting page HTML for: {}", page_id);

    // 1. OAuth 토큰 가져오기
    let access_token = MCP_CLIENT
        .get_oauth_token()
        .await
        .ok_or("Atlassian OAuth 토큰이 없습니다. Confluence에 먼저 연결해주세요.")?;

    debug!(
        "[Confluence REST] Got OAuth token (length: {})",
        access_token.len()
    );

    // 2. cloudId 가져오기 (accessible resources에서)
    let cloud_id = match get_cloud_id(&access_token).await {
        Ok(id) => {
            debug!("[Confluence REST] Got cloudId: {}", id);
            id
        }
        Err(e) => {
            warn!("[Confluence REST] Failed to get cloudId: {}", e);
            return Err(e);
        }
    };

    // 3. Validate cloud_id before URL construction
    validate_url_segment(&cloud_id, "cloud_id")?;

    // 4. Confluence REST API v2 호출
    // https://developer.atlassian.com/cloud/confluence/rest/v2/api-group-page/#api-pages-id-get
    let url = format!(
        "https://api.atlassian.com/ex/confluence/{}/wiki/api/v2/pages/{}?body-format=storage",
        cloud_id, page_id
    );
    debug!("[Confluence REST] Calling API: {}", url);

    let response = HTTP_CLIENT
        .get(&url)
        .header("Authorization", format!("Bearer {}", access_token))
        .header("Accept", "application/json")
        .send()
        .await
        .map_err(|e| format!("Confluence API 요청 실패: {}", e))?;

    let status = response.status();
    debug!("[Confluence REST] Response status: {}", status);

    if !status.is_success() {
        let body = response.text().await.unwrap_or_default();
        warn!("[Confluence REST] Error response: {}", body);
        return Err(format!("Confluence API 오류 ({}): {}", status, body));
    }

    let api_response: ConfluenceApiPageResponse = response
        .json()
        .await
        .map_err(|e| format!("Confluence API 응답 파싱 실패: {}", e))?;

    let body = api_response
        .body
        .and_then(|b| b.storage)
        .map(|s| s.value)
        .unwrap_or_default();

    info!(
        "[Confluence REST] Success! Title: {}, Body length: {}",
        api_response.title,
        body.len()
    );

    Ok(ConfluencePageContent {
        page_id: api_response.id,
        title: api_response.title,
        body,
    })
}

/// Confluence 페이지를 ADF로 가져와 프론트엔드 원문 패널에 로드
///
/// tauri-testing-mcp(oddeyes_load_confluence_page)에서 호출.
/// 실제 변환(ADF→TipTap)과 store 업데이트는 프론트엔드(loadAdfAsSourceDocument)에서 수행하므로
/// 이 command는 MCP call_tool 결과를 그대로 반환.
#[tauri::command]
pub async fn load_confluence_page_as_source(page_url: String) -> Result<serde_json::Value, String> {
    use crate::mcp::client::MCP_CLIENT;

    // pageId 추출 (숫자 시퀀스)
    let page_id = extract_page_id_from_url(&page_url)
        .ok_or_else(|| format!("Confluence 페이지 ID를 추출할 수 없습니다: {}", page_url))?;

    validate_url_segment(&page_id, "page_id")?;

    let access_token = MCP_CLIENT
        .get_oauth_token()
        .await
        .ok_or("Atlassian OAuth 토큰이 없습니다. Settings에서 Confluence에 먼저 연결해주세요.")?;

    let cloud_id = get_cloud_id(&access_token).await?;
    validate_url_segment(&cloud_id, "cloud_id")?;

    // ADF 형식으로 페이지 요청
    let url = format!(
        "https://api.atlassian.com/ex/confluence/{}/wiki/api/v2/pages/{}?body-format=atlas_doc_format",
        cloud_id, page_id
    );

    let response = HTTP_CLIENT
        .get(&url)
        .header("Authorization", format!("Bearer {}", access_token))
        .header("Accept", "application/json")
        .send()
        .await
        .map_err(|e| format!("Confluence API 요청 실패: {}", e))?;

    let status = response.status();
    if !status.is_success() {
        let body = response.text().await.unwrap_or_default();
        return Err(format!("Confluence API 오류 ({}): {}", status, body));
    }

    let json: serde_json::Value = response
        .json()
        .await
        .map_err(|e| format!("Confluence API 응답 파싱 실패: {}", e))?;

    Ok(json)
}

/// URL에서 Confluence pageId(숫자) 추출
/// /pages/123456 또는 ?pageId=123456 패턴 지원
fn extract_page_id_from_url(url: &str) -> Option<String> {
    // /pages/123456 경로 패턴
    if let Some(idx) = url.find("/pages/") {
        let after = &url[idx + "/pages/".len()..];
        let id: String = after.chars().take_while(|c| c.is_ascii_digit()).collect();
        if !id.is_empty() {
            return Some(id);
        }
    }
    // ?pageId=123456 또는 &pageId=123456 쿼리 패턴
    for param in url.split(['?', '&']) {
        if let Some(val) = param.strip_prefix("pageId=") {
            let id: String = val.chars().take_while(|c| c.is_ascii_digit()).collect();
            if !id.is_empty() {
                return Some(id);
            }
        }
    }
    None
}

/// cloudId 가져오기 (캐시 없이 매번 조회 - 간단한 구현)
async fn get_cloud_id(access_token: &str) -> Result<String, String> {
    let url = "https://api.atlassian.com/oauth/token/accessible-resources";

    let response = HTTP_CLIENT
        .get(url)
        .header("Authorization", format!("Bearer {}", access_token))
        .header("Accept", "application/json")
        .send()
        .await
        .map_err(|e| format!("Accessible resources 요청 실패: {}", e))?;

    if !response.status().is_success() {
        let status = response.status();
        let body = response.text().await.unwrap_or_default();
        return Err(format!("Accessible resources 오류 ({}): {}", status, body));
    }

    let resources: Vec<AccessibleResource> = response
        .json()
        .await
        .map_err(|e| format!("Accessible resources 파싱 실패: {}", e))?;

    resources
        .first()
        .map(|r| r.id.clone())
        .ok_or_else(|| "Atlassian cloudId를 찾을 수 없습니다".to_string())
}
