...
[ ] 5.2 로컬 용어집(Glossary) “RAG” — 단계형(룰 기반 → 랭킹/벡터)
설명: 한 번에 “벡터 RAG”로 가지 말고, 먼저 로컬 DB 기반의 실사용 가능한 주입 루프를 완성한다.
...
- 2차(후속): FTS5/BM25 등 랭킹 강화(선택)
- 3차(후속): 임베딩/벡터(선택) — 비용/복잡도 대비 효과 검증 후
