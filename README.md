...
### UI / UX
- **Hybrid Panel Layout**: Editor + Settings/Review 사이드바 + 플로팅 Chat ✅
  - **Settings/Review 사이드바**: 고정 사이드바 (탭 전환, 드래그로 너비 조정 280-600px)
  - **플로팅 Chat 패널**: react-rnd 기반 (드래그 이동, 8방향 리사이즈)
  - **플로팅 Chat 버튼**: FAB 스타일 (드래그 위치 변경, 더블클릭 리셋)
...
├── src/                          # Frontend (React)
│   ├── components/               # UI 컴포넌트
│   │   ├── chat/                 # 채팅 관련 UI (ChatContent 등)
│   │   ├── editor/               # 에디터 관련 UI
│   │   ├── layout/               # 레이아웃/툴바
│   │   ├── panels/               # Source/Target/SettingsSidebar/FloatingChatPanel
│   │   ├── review/               # 검수 UI (ReviewPanel, ReviewResultsTable)
│   │   └── ui/                   # 공통 UI (FloatingChatButton 등)
